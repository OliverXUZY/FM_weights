## copy from https://github.com/JiajunSong629/uncover-hidden-geometry/blob/main/src/model.py
# src/model.py
import os
import math
import numpy as np
...
class Bert(nn.Module):
    ...
    Corrupts the inputs by using random masking, more precisely, during pretraining,
    ...
    def generate_hiddens(self, idx):
        ...
        self.model.eval()
        hiddens = self.model(idx).hidden_states
        return np.array(
            [h.squeeze().numpy(force=True) for h in hiddens]
        )  # shape (L, B, T, C)
...
class RoFormer(nn.Module):
    ...
    The proposed RoPE encodes absolute positional information with rotation matrix and naturally
    incorporates explicit relative position dependency in self-attention formulation.
    Notably, RoPE comes with valuable properties such as flexibility of being expand to any sequence
    lengths, decaying inter-token dependency with increasing relative distances, and capability of
    equipping the linear self-attention with relative position encoding
    """

    def __init__(self) -> None:
        super().__init__()
        from transformers import RoFormerForCausalLM, RoFormerTokenizerFast

        ckpt = "junnyu/roformer_chinese_base"
...
class Bloom(nn.Module):
    """
    https://huggingface.co/docs/transformers/main/en/model_doc/bloom#transformers.BloomForCausalLM
    """
    ...
    def __init__(self) -> None:
        ...
        self.model = BloomForCausalLM.from_pretrained(ckpt, output_hidden_states=True)
        self.tokenizer = BloomTokenizerFast.from_pretrained(ckpt)
